...
struct App
{
    ...
    // Grid map & obstacles
    std::vector<std::vector<int>> m_GridFlags;  // boxid가 기록됨 0 = empty
	...
	void ProcessNetwork()
	{
		...
		while (m_Client->PopLine(line))
		{
			...
			// ---------------------------------
            // OBSTACLE_SPAWN <cellX> <cellZ>
			// ---------------------------------
            else if (cmd == "OBSTACLE_SPAWN")
            {
				int key, boxid, x, z;
				iss >> key >> boxid >> x >> z; // x,z 는 셀 중앙 포지션
                ...
                //m_ObstacleBoxes.push_back()
                auto& grid = m_GridFlags;
                int gx, gz;
                if (!WorldToGrid(pos, gx, gz)) return;
				grid[gz][gx] = boxid;
				Box obstacle; obstacle.Init(pos, m_CellSize);
                obstacle.m_BoxId = boxid;
				m_ObstacleBoxes.push_back(obstacle);
            
            }
			// ---------------------------------
			// OBSTACLE_DELETE <cellX> <cellZ>
			// ---------------------------------
			else if (cmd == "OBSTACLE_DELETE")
			{
				int gx,gz,boxid;
                iss >> gx >> gz >> boxid; // x,z 는 셀 중앙 포지션

                auto& grid = m_GridFlags;
				grid[gz][gx] = 0;

				auto it = std::find_if(
					m_ObstacleBoxes.begin(),
					m_ObstacleBoxes.end(),
					[&](const Box& obs)
					{
						return obs.m_BoxId == boxid;
					});

				if (it == m_ObstacleBoxes.end())
					return;

				m_ObstacleBoxes.erase(it);
	...
	void SendSpawnObstacleToServer(const Vector3& cellCenter)
	{
		int cellX = static_cast<int>(cellCenter.x - 0.5f / m_CellSize);
		int cellZ = static_cast<int>(cellCenter.z - 0.5f / m_CellSize);
	...
	void SendDeleteObstacleToServer(const int& boxid, int& gx, int& gz)
	{
        m_ObstacleBoxes;
	...
	void SendDeleteRequestToServer()
	{
		std::string msg =
			"DELETE " +
			std::to_string(m_MySessionKey) + "\n";
    ...
    void ToggleObstacleAtMouse(int mx, int my)
    {
        ...
        Vector3 cellPos = SnapToCellCenter(hit); // 셀 중앙 포지션
        ...
        {
            // 서버에 장애물 스폰해달라고 요청
            SendSpawnObstacleToServer(cellPos);
        }
        else
        {
            // 서버에 장애물 제거해달라고 요청
            SendDeleteObstacleToServer(grid[gz][gx], gx, gz);
        }
    }
